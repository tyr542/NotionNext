...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  if (pageIds?.length === 0) {
    ...
    console.log('[DEBUG] 有效Page数量', pageIds?.length)
  }
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    if (i < 3) {
      const raw = block[id]
      console.log(`[DEBUG] block[${i}] id=${id} keys=${raw ? Object.keys(raw).join(',') : 'null'} normalized=${!!pageBlock}`)
    }
  }
  console.log(`[DEBUG] blockIdsNeedFetch: ${blockIdsNeedFetch.length}/${pageIds.length}`)

  // 2️⃣ fetch 缺失的 blocks
  const fetchedBlocks = await fetchInBatches(blockIdsNeedFetch)
  // DEBUG: fetchedBlocks 结构
  const fetchedKeys = Object.keys(fetchedBlocks || {})
  console.log(`[DEBUG] fetchedBlocks count: ${fetchedKeys.length}`)
  if (fetchedKeys.length > 0) {
    const sk = fetchedBlocks[fetchedKeys[0]]
    console.log(`[DEBUG] sample fetchedBlock keys: ${sk ? Object.keys(sk).join(',') : 'null'} sample: ${JSON.stringify(sk)?.slice(0, 500)}`)
  }
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    if (!pageBlock) {
      if (i < 3) console.warn(`[DEBUG] skip block[${i}]:`, id, JSON.stringify(rawBlock)?.slice(0, 400))
      continue
    }
    ...
    if (properties) {
      ...
      if (collectionData.length <= 2) {
        console.log(`[DEBUG] article[${collectionData.length-1}]: type=${properties.type} status=${properties.status} slug=${properties.slug}`)
      }
    }
  }
  console.log(`[DEBUG] collectionData total: ${collectionData.length}`)
