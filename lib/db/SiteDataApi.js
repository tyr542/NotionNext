...
function handleDataBeforeReturn(db) {
  ...
  if (db.invisiblePages) {
    db.invisiblePages = cleanPages(db.invisiblePages, db.tagOptions)
  }
...
function stripRichListFields(page) {
  ...
  const lightweightPage = { ...page }
  delete lightweightPage.blockMap
  delete lightweightPage.content
  delete lightweightPage.toc
