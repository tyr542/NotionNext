...
import { fetchSitePageviews } from '@/lib/plugins/ga-pageviews'
...
import { sanitizeListItemForProps } from '../utils/listPageProps'
import { zonedDateTimeToUtcMs } from '../utils/timezone'
...
} from './notion/publicContent'
import { findAccessiblePage } from './notion/findAccessiblePage'
...
export async function fetchGlobalAllData({
  ...
  // 注入 GA 站點流量（SSG / ISR 階段呼叫一次，失敗時為 null，前端 fallback 顯示 '—'）
  try {
    const gaStats = await fetchSitePageviews()
    if (gaStats) {
      data.sitePv = gaStats.sitePv
      data.siteUv = gaStats.siteUv
    } else {
      data.sitePv = null
      data.siteUv = null
    }
  } catch (e) {
    console.warn('[fetchGlobalAllData] GA 取得失敗', e?.message || e)
    data.sitePv = null
    data.siteUv = null
  }
...
export async function resolvePostProps({
   ...
   */
  post = findAccessiblePage({
    fullSlug,
    publicPages: props?.allPages,
    invisiblePages: props?.invisiblePages
  })
  ...
  if (post) {
    ...
    props.post.blockMap = shrinkRecordMapForPageProps(props.post?.blockMap)
    props.notice = createNoticePreview(props.notice)
  } else {
    props.post = null
  }

  delete props.invisiblePages
...
async function convertNotionToSiteData(
  ...
  // 文章计数
  const postCount = collectionData.filter(isPublishedPost).length

  // 查找所有的Post和Page
  const allPages = collectionData.filter(isPublishedPublicContent)
  const invisiblePages = includeInvisiblePages
    ? collectionData.filter(isInvisiblePublicContent)
    : null
  ...
  const customNav = getCustomNav({
    allPages: collectionData.filter(isPublishedPage)
  })
  // 新的菜单
...
function handleDataBeforeReturn(db) {
  ...
  db.allNavPages = cleanListPages(db?.allNavPages, db.tagOptions)
  db.allPages = cleanListPages(db.allPages, db.tagOptions)
  if (db.invisiblePages) {
    db.invisiblePages = cleanListPages(db.invisiblePages, db.tagOptions)
  }
  db.latestPosts = cleanListPages(db.latestPosts, db.tagOptions)
  db.latestUpdatedPosts = cleanListPages(db.latestUpdatedPosts, db.tagOptions)
  // 必须在使用完毕后才能进行清理
...
function cleanListPages(allPages, tagOptions) {
  return cleanPages(allPages, tagOptions).map(stripRichListFields)
}

function stripRichListFields(page) {
  return sanitizeListItemForProps(page)
}
...
function getLatestPosts({ allPages, from, latestPostCount }) {
  const allPosts = allPages?.filter(isPublishedPost)
...
function getTimestamp(date, time = '00:00', time_zone) {
  if (!date) return null
  return zonedDateTimeToUtcMs(date, time, time_zone || 'Asia/Shanghai')
}
...
export function getNavPages({ allPages }) {
  const allNavPages = allPages?.filter(isPublishedPost)
