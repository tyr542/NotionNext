...
import { processPostData } from '../utils/post-server'
import { adapterNotionBlockMap } from '../utils/notion.util'
import { getLatestUpdatedPosts } from '../utils/postSort'
...
async function convertNotionToSiteData(
  ...
  const latestUpdatedPosts = getLatestUpdatedPosts(allPages, 6)
...
function getCustomMenu({ collectionData, NOTION_CONFIG }) {
  ...
  // 依 date 欄位「由新到舊」決定選單排序（date 越新 = 越靠左）。
  // 修正：原本直接吃 Notion 回傳的原始列順序，無法穩定控制（日期/手動拖曳都不生效）。
  // 注意：SubMenu 會掛到緊鄰其前方的 Menu，所以每個父 Menu 的 date 必須比它的 SubMenu 新，
  // 且父選單與其 SubMenu 的 date 區間中間不可插入其他 Menu。
  menuPages.sort((a, b) => {
