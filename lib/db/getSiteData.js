...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  const collection = normalizeCollection(
    Object.values(pageRecordMap.collection)[0]
  )
  ...
  for (let i = 0; i < pageIds.length; i++) {
    const id = pageIds[i]
    const value = normalizePageBlock(block[id])
    if (!value) {
  ...
  for (let i = 0; i < pageIds.length; i++) {
    const id = pageIds[i]
    const rawBlock = block[id] || fetchedBlocks[id]
    const value = normalizePageBlock(rawBlock)
    if (!value) {
      console.warn(`[Normalize] Skip block ${id}: cannot resolve page block`)
      continue
    }
    const properties =
