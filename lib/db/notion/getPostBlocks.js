...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      await hydrateWrappedCollectionQueries(pageData)
...
function getCollectionQueryTargets(recordMap) {
  const targets = []
  const seen = new Set()

  Object.values(recordMap?.block || {}).forEach(blockRecord => {
    const block = unwrapRecordValue(blockRecord)
    ...
    if (!collectionId || !Array.isArray(block.view_ids)) {
      return
    }

    block.view_ids.forEach(collectionViewId => {
      if (!collectionViewId) {
        return
      }

      const key = `${collectionId}:${collectionViewId}`
      if (seen.has(key)) {
        return
      }
...
async function hydrateWrappedCollectionQueries(recordMap) {
  const targets = getCollectionQueryTargets(recordMap)

  if (targets.length === 0 || typeof notionAPI.__call !== 'function') {
    return recordMap
  }
  ...
  await Promise.all(
    targets.map(
      async ({ collectionId, collectionViewId, collectionView, spaceId }) => {
        try {
          ...
          console.warn(
            '[hydrateWrappedCollectionQueries] getCollectionData failed:',
            collectionId,
            collectionViewId,
            error
          )
...
function sanitizeBlockUrls(blockValue) {
  ...
  const invalidUrlPlaceholder = 'https://via.placeholder.com/1x1?text=Invalid+Image'
  ...
  const fixUrl = (url) => {
    ...
    try {
      ...
      if (!['http:', 'https:'].includes(parsedUrl.protocol)) {
        console.warn('[Sanitize URL] Unsafe URL protocol replaced:', url)
