...
const isNotionImageProxy = urlObj => {
  if (!urlObj) {
    return false
  }
  ...
  try {
    return (
      urlObj.origin === new URL(BLOG.NOTION_HOST).origin &&
      urlObj.pathname.startsWith('/image/')
    )
  } catch (error) {
    return false
  }
...
const compressImage = (image, width, quality = 50, fmt = 'webp') => {
  ...
  const maxWidth = Number(siteConfig('IMAGE_COMPRESS_WIDTH')) || 1080
  if (!width || width === 0) {
    width = maxWidth
  } else {
    width = Math.min(Number(width) || maxWidth, maxWidth)
  }
