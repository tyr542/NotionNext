...
export class Validator {
  ...
  static isValidEmail(email) {
    ...
    if (!REGEX_PATTERNS.email.test(normalizedEmail)) return false
...
export class Sanitizer {
  ...
  static sanitizeXss(str) {
    ...
    if (/^(javascript|vbscript|data:text\/html)\s*:/i.test(trimmed)) {
      return ''
    }
    ...
    XSS_PATTERNS.forEach(pattern => {
      const nextCleaned = cleaned.replace(pattern, '')
    ...
    if (wasSanitized && /^(alert|confirm|prompt)\s*\(.*\)$/i.test(cleaned)) {
      return ''
    }
